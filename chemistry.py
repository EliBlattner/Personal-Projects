...
def make_periodic_table():
    periodic_table_dict = {"Ac" : 	["Actinium", 	227],
    "Ag" 	:["Silver", 	107.8682],
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    print(f"Your molar mass is {total_mass}g, which means:")
    print()
    return total_mass


def main():
    user_input_formula = input("Please enter a chemical formula: ")
    user_input_mass = float(input("\nWhat is the mass(g) of the chemical sample?: "))
    ptable = make_periodic_table()
    separated_list = parse_formula(user_input_formula, ptable)
    
    moles = user_input_mass / compute_molar_mass(separated_list, ptable)
    print(f"Your sample contains {moles:.3f} moles.")
